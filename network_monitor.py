...
def init_db():
    ...
    try:
        ...
        conn = psycopg2.connect(
            host=host, port=port, database=DB_NAME, user=DB_USER, password=DB_PASSWORD
        )
        cur = conn.cursor()
...
def get_report_data(hours):
    ...
    try:
        ...
        summary = {
            ...
            "Slow Download Incidents": sum(
                1 for speed in download_speeds if speed < DOWNLOAD_THRESHOLD
            ),
            "Slow Upload Incidents": sum(
                1 for speed in upload_speeds if speed < UPLOAD_THRESHOLD
            ),
        }
        ...
        for r in results:
            if (
                r["download_speed"] < DOWNLOAD_THRESHOLD
                or r["upload_speed"] < UPLOAD_THRESHOLD
            ):
                incidents.append(
                    [
                        r["timestamp"].strftime("%Y-%m-%d %H:%M:%S"),
                        (
                            f"{r['download_speed']:.2f} Mbps"
                            if r["download_speed"] < DOWNLOAD_THRESHOLD
                            else "OK"
                        ),
                        (
                            f"{r['upload_speed']:.2f} Mbps"
                            if r["upload_speed"] < UPLOAD_THRESHOLD
                            else "OK"
                        ),
                    ]
                )
...
def perform_network_check():
    ...
    if all(v is not None for v in [download_speed, upload_speed, latency, ip_address]):
        ...
        logging.info(
            f"Network check completed - Download: {download_speed:.2f} Mbps, Upload: {upload_speed:.2f} Mbps, Latency: {latency:.2f} ms"
        )
    else:
...
def main():
    ...
    schedule.every(SUMMARY_INTERVAL_HOURS).hours.do(
        lambda: send_email_report(SUMMARY_INTERVAL_HOURS)
    )
